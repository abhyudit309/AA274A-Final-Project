...
NoiseParams = {'Sigma0': 0.01*np.eye(3),  #0.01 # initial state covariance (x0 comes from ground truth; nonzero in case of timing mismatch)
               'R': 0.1*np.eye(2),   #0.1 # control noise covariance (corresponding to dt = 1 second)
               'var_theta': 0.03,     #0.03 # laser scan noise variance in theta measurement (per point)
               'var_rho': 0.05,      #0.05 # laser scan noise variance in rho measurement (per point)
