...
class Ekf(object):
    ...
    def transition_update(self, u, dt):
        ...
        self.x = g
        self.Sigma = np.matmul(Gx, np.matmul(self.Sigma, np.transpose(Gx))) + \
                     dt * np.matmul(Gu, np.matmul(self.R, np.transpose(Gu)))
    ...
    def compute_predicted_measurements(self):
        ...
        for j in range(J):
            ...
            if j >= 2:
                ...
                Hx[1, idx_j] = self.x[0] * np.sin(alpha) - self.x[1] * np.cos(alpha) - self.tf_base_to_camera[0] * np.sin(self.x[2] - alpha) - self.tf_base_to_camera[1] * np.cos(self.x[2] - alpha)
