...
class Navigator:
    ...
    def stop_sign_detected_callback(self, msg):
        ...
        if dist > 0 and dist < self.stop_min_dist:
            rospy.sleep(5)
            rospy.loginfo("----------stopped at stop sign ---------------")
    ...
    def object_detected_callback(self, msg):
        if msg.name == "stop_sign" and msg.confidence >= 0.85:
            #if self.stop_time + self.STOP_TIME + self.CROSS_TIME < rospy.get_time():
            self.stop_time = rospy.get_rostime()
            rospy.loginfo('----------stopped at stop sign ---------------={}'.format(msg.confidence))
    ...
    def has_stopped(self):
        ...
        return self.mode == Mode.STOP and \
               rospy.get_rostime() - self.stop_time > rospy.Duration.from_sec(self.STOP_TIME)
    ...
    def has_crossed(self):
        ...
        return self.mode == Mode.CROSS and \
               rospy.get_rostime() - self.cross_start > rospy.Duration.from_sec(self.CROSS_TIME)
